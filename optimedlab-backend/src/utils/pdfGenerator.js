...
const generateQuotePDF = (quote, client, companyInfo = {}) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        .fillColor("#047857") // Professional Excel Green / Emerald
        .fontSize(24)
        .font("Helvetica-Bold")
      ...
      doc.font("Helvetica").fontSize(10).text(client.name, 50, 165);
      ...
      const signatureY = doc.page.height - 150; // Position from bottom
      ...
      doc.text("Date :", 300, signatureY - 15);
      doc.text("_____________", 340, signatureY - 15);
...
const generateInvoicePDF = (invoice, client, companyInfo = {}) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      // 2. Client Information – ALL FOUR FIELDS INLINE
      doc
        .fillColor("#374151")
        .fontSize(11)
        .font("Helvetica-Bold")
      ...
      let clientY = 175; // starting Y for client details
      doc.fontSize(10).font("Helvetica");

      // Build one line with all available fields: name, company, email, phone
      let parts = [];
      if (client.name) parts.push(`Client name : ${client.name}`);
      if (client.company) parts.push(`Company : ${client.company}`);
      if (client.email) parts.push(`Gmail : ${client.email}`);
      ...
      // 3. Data Table (positioned dynamically after client info)
      const tableTop = clientY + 30; // 30px spacing after client details
